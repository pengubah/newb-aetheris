...
nl_skycolor nlOverworldSkyColors(nl_environment env) {
  ...
  float dawnMask = dawn*(1.0-0.35*night);
  s.zenith = mix(s.zenith,dawnZenith,dawnMask*0.58);
  s.horizon = mix(s.horizon,dawnHorizon,dawnMask*0.82);
  s.horizonEdge = mix(s.horizonEdge,dawnEdge,dawnMask);
...
vec3 renderOverworldSky(nl_skycolor skyCol,nl_environment env,vec3 viewDir,bool isSkyPlane) {
  ...
  // warm sun halo and cool moon halo
  float sunDisc = pow(max(dot(env.sunDir,viewDir),0.0),96.0);
  float sunHalo = pow(max(dot(env.sunDir,viewDir),0.0),8.0);
  float moonHalo = pow(max(dot(env.moonDir,viewDir),0.0),12.0);
  float dawnHalo = smoothstep(0.0,1.0,1.0-abs(env.dayFactor));
  sky += sunHalo*0.16*mix(NL_DAWN_SUNLIGHT_COL,NL_NOON_SUNLIGHT_COL,smoothstep(-0.05,0.55,env.dayFactor));
  sky += sunDisc*1.7*mix(NL_DAWN_SUNLIGHT_COL,NL_NOON_SUNLIGHT_COL,smoothstep(0.0,0.55,env.dayFactor));
  sky += moonHalo*0.16*NL_NIGHT_MOONLIGHT_COL*(1.0-env.rainFactor)*step(env.dayFactor,0.0);
